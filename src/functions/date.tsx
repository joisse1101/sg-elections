...
let month_ms = 30.437 * day_ms
let year_ms = 365.25 * day_ms
...
export function timeDiff_ymd(date_now: Date, date_past: Date) {
    let years = year_diff(date_now, date_past)
    date_past.setFullYear(date_past.getFullYear() + years)
    let months = month_diff(date_now, date_past)
    date_past.setMonth(date_past.getMonth() + months)
    let days = day_diff(date_now, date_past)

    return new Set<number>([years, months, days])
}